...
(function () {
   ...
   * @returns {void}
   */
  function openPanel(slug, title, trigger, pushHistory) {
    var panel = document.getElementById('repo-panel');
    ...
    if (pushHistory) {
      history.pushState({ slug: slug, title: title }, '', 'repos/' + encodeURIComponent(slug) + '/');
    }
    ...
    fetch(FRAGMENT_BASE + encodeURIComponent(slug) + '.html')
      ...
      .catch(function () {
        body.innerHTML =
          '<p class="error-state">Details could not be loaded. ' +
          '<a href="repos/' + encodeURIComponent(slug) + '/">Open the full page instead.</a></p>';
      });
  }
  ...
  function initPanel() {
    ...
    document.addEventListener('click', function (event) {
      ...
      event.preventDefault();
      openPanel(slug, titleElement ? titleElement.textContent : slug, titleElement || trigger, true);
    });
    ...
    window.addEventListener('popstate', function (event) {
      if (panelIsOpen) {
        ...
        openPanel(event.state.slug, event.state.title || event.state.slug, null, false);
      }
    });
  }
