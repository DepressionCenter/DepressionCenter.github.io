...
# resize_nonthumbs.py - Resize and compress the full-size repository preview images.
...
"""Normalize the hand-made full-size previews kept in images/repo-previews/.

Run this after adding a new preview by hand. Images pulled automatically from other
...
from imagelib import FULL_MAX_WIDTH, normalize_preview, save_png
...
files = [p for p in sorted(ROOT.glob(PATTERN)) if not p.name.endswith("-thumb.png")]
if not files:
...
for path in files:
    ...
    try:
        width, height = save_png(normalize_preview(image, FULL_MAX_WIDTH), path)
